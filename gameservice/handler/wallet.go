...
func GetStatement(c *fiber.Ctx) error {
	...
	database.Database.Find(&statement)
...
func UpdateStatement(c *fiber.Ctx) error {
    ...
    if err_ := database.Database.Where("uid = ? ", BankStatement.Uid).First(&bankstatement).Error; err_ != nil {
	...
	//result := database.Database.Create(&BankStatement); 
	...
	// } else {
		...
		if err := database.Database.Model(&bankstatement).Updates(updates).Error; err != nil {
			return c.JSON(fiber.Map{
				"status": false,
				"message": err,
				"data": fiber.Map{ 
		...
		// _err := repository.UpdateUserFields(database.Database, BankStatement.Userid, updates) // อัปเดตยูสเซอร์ที่มี ID = 1
		// fmt.Println(_err)
		// if _err != nil {
...
func AddStatement(c *fiber.Ctx) error {
    ...
    if err_ := database.Database.Where("walletid = ? ", BankStatement.Userid).First(&users).Error; err_ != nil {
		return c.JSON(fiber.Map{
			"status": false,
			"message": err_,
			"data": fiber.Map{ 
...
func GetBankStatement(c *fiber.Ctx) error {
	...
	if err := c.BodyParser(body); err != nil {
		...
		if body.Status == "all" {
			database.Database.Debug().Select("uid,userid,createdAt,accountno,transactionamount,channel,walletid,status").Where(" channel= ? AND  DATE(createdat) BETWEEN ? AND ? ", channel, startDateStr, endDateStr).Order("id desc").Find(&statements)
		} else {
			database.Database.Debug().Select("uid,userid,createdAt,accountno,transactionamount,channel,walletid,status").Where(" channel= ? AND  DATE(createdat) BETWEEN ? AND ? and status = ?",channel, startDateStr, endDateStr,body.Status).Order("id desc").Find(&statements)
		}
	  ...
	  // วนลูปเพื่อประมวลผลแต่ละรายการ
	   for i, transaction := range statements {
		 ...
		 //users models.Users
		 var users models.Users
		 database.Database.Debug().Where("walletid = ?",transaction.Walletid).Find(&users)
